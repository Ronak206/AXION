...
class NeuralNetChatbot {
  ...
  public:
    ...
    string predict(string userInput) {
        ...
        return response.empty() ? "I'm not sure about that. Can you explain more?" :
               response;
    }
    ...
    string processSubstrings(vector<string> tokens, int depth) {
        ...
        if (!bestMatch.empty() && responseMap.find(bestMatch) != responseMap.end()) {
            ...
            return responseMap[bestMatch] + (remainingResponse.empty() ? "" : " " +
                                             remainingResponse);
        } else {
    ...
    vector<string> removeSubstring(const vector<string>& tokens,
